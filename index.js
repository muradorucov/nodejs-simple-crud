...
app.put("/ptoduct/:id", function (req, res) {
    const idOfProduct = parseInt(req.params.id);
    const ProductIdx = products.findIndex((product) => product.id === idOfProduct);

    if (ProductIdx !== -1) {
        const oldProduct = products[ProductIdx];
        products[ProductIdx] = { ...oldProduct, ...req.body };
